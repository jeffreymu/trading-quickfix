...
public class FIXMessageUtil {
	...
	private static final int MAX_FIX_FIELDS = 2000; // What we think the ID of
													// the last fix field is
	public static final Pattern optionSymbolPattern = Pattern.compile("(\\w{1,3})\\+(\\w)(\\w)"); //$NON-NLS-1$
	private static Logger logger = LoggerFactory.getLogger(FIXMessageUtil.class);
	...
	private static boolean msgTypeHelper(Message fixMessage, String msgType) {
		if (fixMessage != null) {
			...
			if (header.isSetField(msgTypeField)) {
				...
				logger.error("Fail to get the head of message " + fixMessage.toString());
				return false;
			}
		}
		return false;
	...
	public static boolean isDerivativeSecurityList(Message message) {
	...
	public static boolean isSecurityListRequest(Message inMessage) {
		return msgTypeHelper(inMessage, MsgType.SECURITY_LIST_REQUEST);
	}
	 ...
	 *             if the given message does not contain market depth
	 *             information
	 */
	private static int getMarketDepth(Message inMessage) throws FieldNotFound {
		return inMessage.getInt(MarketDepth.FIELD);
	}

	/**
	 * See
	 * {@link #fillFieldsFromExistingMessage(quickfix.Message, quickfix.Message, boolean, java.util.Set)}
	 * . Invokes this api with a null inclusion set.
	 * 
	 * @param outgoingMessage
	 *            The message whose fields need to be filled.
	 * @param existingMessage
	 *            The message whose fields need to be copied.
	 * @param onlyCopyRequiredFields
	 *            if only required fields should be copied.
	 */
	...
	/**
	 * See
	 * {@link #fillFieldsFromExistingMessage(quickfix.Message, quickfix.Message, DataDictionary, boolean, java.util.Set)}
	 * . Invokes this api with a null inclusion set.
	 * 
	 * @param outgoingMessage
	 *            The message whose fields need to be filled.
	 ...
	 */
	public static void fillFieldsFromExistingMessage(Message outgoingMessage, Message existingMessage,
			DataDictionary dict, boolean onlyCopyRequiredFields) {
		fillFieldsFromExistingMessage(outgoingMessage, existingMessage, dict, onlyCopyRequiredFields, null);
	}

	/**
	 * Helper method to extract all useful fields from an existing message into
	 * another message This is usually called when the "existing" message is
	 * malformed and is missing some fields, and an appropriate "reject" message
	 * needs to be sent. Can't say we are proud of this method - it's rather a
	 * kludge. Goes through all the required fields in "outgoing" message, and
	 * ignores any missing ones Skips over any of the outgoing fields that have
	 * already been set
	 * 
	 * Use cases: an order comes in missing a Side, so we need to create an
	 * ExecutionReport that's a rejection, and need to extract all the other
	 * fields (ClOrdId, size, etc) which may or may not be present since the
	 * order is malformed <strong>warning: ignores groups</strong>
	 * 
	 * @param outgoingMessage
	 *            The message whose fields need to be filled.
	 * @param existingMessage
	 ...
	 *            if only required fields should be copied.
	 * @param inclusionSet
	 *            the set of fields that should be copied. Can be null. If not
	 *            null, only the fields present in this set are copied.
	 */
	public static void fillFieldsFromExistingMessage(Message outgoingMessage, Message existingMessage,
			boolean onlyCopyRequiredFields, Set<Integer> inclusionSet) {
		fillFieldsFromExistingMessage(outgoingMessage, existingMessage, CurrentFIXDataDictionary
				.getCurrentFIXDataDictionary().getDictionary(), onlyCopyRequiredFields, inclusionSet);
	}
	 ...
	 */
	public static void fillFieldsFromExistingMessage(Message outgoingMessage, Message existingMessage,
			DataDictionary dict, boolean onlyCopyRequiredFields, Set<Integer> inclusionSet) {

		String msgType = null;
		try {
			msgType = outgoingMessage.getHeader().getString(MsgType.FIELD);
		} catch (FieldNotFound ex) {
			logger.error("Outgoing message did not have valid MsgType.", ex);
			return;
		}
		...
		while (fieldItr.hasNext()) {
			...
			if ((!onlyCopyRequiredFields || dict.isRequiredField(msgType, fieldInt))
					&& !outgoingMessage.isSetField(fieldInt) && dict.isMsgField(msgType, fieldInt)) {
				try {
	...
	public static String getTextOrEncodedText(Message aMessage, String defaultString) {
		...
		if (aMessage.isSetField(Text.FIELD)) {
			...
			try {
				text = aMessage.getString(EncodedText.FIELD); // i18n_string
																// todo use the
																// correct
																// MessageEncoding
																// value
	 ...
	 * 
	 * @param msgType
	 *            the msgType of the request message
	 * @return the field that represents the request/response correlation ID in
	 *         the request message
	 */
	public static StringField getCorrelationField(FIXVersion version, String msgType) {
		...
		if (MsgType.COLLATERAL_REQUEST.equals(msgType) || MsgType.COLLATERAL_RESPONSE.equals(msgType)) {
			...
			reqIDField = new ConfirmReqID();
				...
				|| MsgType.DERIVATIVE_SECURITY_LIST.equals(msgType)) {
			reqIDField = new SecurityReqID();
		} else if (MsgType.MARKET_DATA_REQUEST.equals(msgType)
				|| MsgType.MARKET_DATA_INCREMENTAL_REFRESH.equals(msgType)
				|| MsgType.MARKET_DATA_REQUEST_REJECT.equals(msgType)
				|| MsgType.MARKET_DATA_SNAPSHOT_FULL_REFRESH.equals(msgType)) {
			...
			reqIDField = new NetworkRequestID();
				...
				|| MsgType.POSITION_MAINTENANCE_REPORT.equals(msgType) || MsgType.REQUEST_FOR_POSITIONS.equals(msgType)
				|| MsgType.REQUEST_FOR_POSITIONS_ACK.equals(msgType) || MsgType.POSITION_REPORT.equals(msgType)) {
			reqIDField = new PosReqID();
		} else if (MsgType.QUOTE_REQUEST.equals(msgType) || MsgType.QUOTE_REQUEST_REJECT.equals(msgType)
				|| MsgType.QUOTE.equals(msgType)) {
			...
			reqIDField = new SecurityStatusReqID();
				...
				|| MsgType.SETTLEMENT_INSTRUCTIONS.equals(msgType)) {
			reqIDField = new SettlInstReqID();
		} else if (MsgType.TRADE_CAPTURE_REPORT_REQUEST.equals(msgType) || MsgType.TRADE_CAPTURE_REPORT.equals(msgType)
				|| MsgType.TRADE_CAPTURE_REPORT_REQUEST_ACK.equals(msgType)) {
			reqIDField = new TradeRequestID();
		} else if (MsgType.TRADING_SESSION_STATUS_REQUEST.equals(msgType)
				|| MsgType.TRADING_SESSION_STATUS.equals(msgType)) {
	...
	public static void mergeMarketDataMessages(Message marketDataSnapshotFullRefresh,
			Message marketDataIncrementalRefresh, FIXMessageFactory factory) {
		if (!isMarketDataSnapshotFullRefresh(marketDataSnapshotFullRefresh)) {
			throw new IllegalArgumentException(
					"Incoming marketDataSnapshotFullRefresh parameter into MarketData merge must be of MarketDataSnapshotFullRefresh type.");
		}
		if (!isMarketDataIncrementalRefresh(marketDataIncrementalRefresh)) {
			throw new IllegalArgumentException(
					"Incoming marketDataIncrementalRefresh parameter into MarketData merge must be of MarketDataIncrementalRefresh type.");
		}
	...
	private static void addGroupsToMap(Message marketDataMessage, FIXMessageFactory factory,
			HashMap<Character, Group> consolidatingSet) {
		try {
	 ...
	 * 
	 * @param inFieldName
	 *            a <code>String</code> value to containing the name of the
	 *            field to create
	 * @return a <code>Field&lt;?&gt;</code> value
	 * @throws NullPointerException
	 *             if <code>inFieldName</code> is null
	 * @throws Exception
	 *             if the value cannot be converted to a field
	 */
	public static Field<?> getQuickFixFieldFromName(String inFieldName) throws Exception {
		if (inFieldName == null) {
			throw new NullPointerException();
		}
		Throwable error = null;
		try {
			// try the easy case first: the given field might be one of the
			// pre-packaged quickfix fields
			return (Field<?>) Class.forName(QUICKFIX_PACKAGE + inFieldName).newInstance();
		} catch (ClassNotFoundException cnfe) {
			// if this exception is thrown, that means that the field does not
			// correspond to a pre-packaged quickfix
			// field
			// see if we can create a custom field - this means that the field
			// name has to be parseable as an int
			try {
				int fieldInt = Integer.parseInt(inFieldName);
				return new CustomField<Integer>(fieldInt, null);
			} catch (Throwable t) {
				error = t;
			}
		} catch (Throwable t) {
		...
		throw new Exception("A FIX field could not be created from " + inFieldName, error);
	}
	 ...
	 *            the FIX Message.
	 * @param inDict
	 *            the data dictionary for the message
	 * 
	...
	/**
	 * Converts the supplied FieldMap to a map with human readable field names
	 * (based on the supplied dictionary) as keys and field values as string
	 * values.
	 * <p>
	 ...
	 */
	private static HashMap<String, String> fieldsToMap(FieldMap inMap, FIXDataDictionary inDict) {
		...
		while (iterator.hasNext()) {
			...
			if (f instanceof StringField) {
				...
				if (inDict != null) {
					...
					if (humanValue != null) {
						value = new StringBuilder().append(humanValue).append("(").append(value). //$NON-NLS-1$
			...
			if (name == null) {
				...
				name = new StringBuilder().append(name).append("(").append(f.getTag()). //$NON-NLS-1$
						append(")").toString(); //$NON-NLS-1$
			}
	 ...
	 * @param qMsgReply
	 * @param qMsg
	 * @param bestDataDictionary
	 */
	public static void fillAllFieldsFromExistingMessage(Message outgoingMessage, Message existingMessage,
			DataDictionary dict) {
		String msgType = null;
		try {
			msgType = outgoingMessage.getHeader().getString(MsgType.FIELD);
