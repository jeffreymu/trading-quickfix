...
/**
 * Analyzes a QuickFIX/J field, producing a human-readable representation of its
 * contents.
 * 
 */

public class AnalyzedField {
	private static Logger logger = LoggerFactory.getLogger(AnalyzedField.class);
	...
	/**
	 * Creates a new analyzed field for the given QuickFIX/J field. The field is
	 * part of the given QuickFIX/J field map, and the map is part of (or same
	 * as) the message with the given type. One data dictionary is used to
	...
	AnalyzedField(DataDictionary nameQDict, DataDictionary scopeQDict, FieldMap qMap, String msgType, Field<?> qField) {
		...
		DataDictionary.GroupInfo info = scopeQDict.getGroup(msgType, getQFieldTag());
		if (info == null) {
			return;
		}
		...
		for (int i = 0; i < count; i++) {
			...
			try {
				...
				logger.error("Group " + i + 1 + " is missing from message scope " + qMap);
				continue;
			}
			mGroups.add(new AnalyzedGroup(nameQDict, info.getDataDictionary(), group, msgType));
		}
	}
	...
	public void print(PrintStream stream, String prefix) {
		stream.print(prefix);
		stream.print(getName() + " [{ " + getQFieldTag() + "}{" + getValue() + ",choice,0#|1#R}] = {"
				+ (getRequired() ? 1 : 0) + "}");
		prefix += ' '; //$NON-NLS-1$
