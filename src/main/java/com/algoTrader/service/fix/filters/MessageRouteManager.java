...
import com.algoTrader.service.fix.quickfix.AlgoFIXException;
...
public class MessageRouteManager {
	...
	public void setRouteMethod(String inMethod) {
		...
		if (routeMethod != null
				&& (!(FIELD_57_METHOD.equals(routeMethod) || FIELD_100_METHOD.equals(routeMethod) || FIELD_128_METHOD
						.equals(routeMethod)))) {
			throw new IllegalArgumentException("Could not recognize route method: " + routeMethod);
		}
	}
	...
	public boolean modifyMessage(Message anOrder, FIXMessageAugmentor augmentor) throws Exception {
		try {
			...
			if (symbolString != null) {
				...
				if (routeMethod != null
						&& (MsgType.ORDER_SINGLE.equals(msgTypeField.getValue())
								|| MsgType.ORDER_CANCEL_REPLACE_REQUEST.equals(msgTypeField.getValue()) || MsgType.ORDER_CANCEL_REQUEST
									.equals(msgTypeField.getValue()))) {
					int periodPosition;
				...
				// don't separate suffixes on Forex orders (SecurityType may not
				// be set on all orders so check explicitly)
				String securityType = (anOrder.isSetField(SecurityType.FIELD)) ? anOrder.getString(SecurityType.FIELD)
						: null;
			...
			throw AlgoFIXException.createFieldNotFoundException(fnfEx, anOrder);
		} catch (Exception ex) {
