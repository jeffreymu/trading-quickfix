...
public class DefaultMessageModifier implements MessageModifier {
	...
	class MessageModifier {
		...
		public void addDefaultField(int field, Object defaultValue, MessageFieldType fieldType) {
			switch (fieldType) {
			case MESSAGE:
				msgFields.put(field, defaultValue);
				break;
			case HEADER:
				headerFields.put(field, defaultValue);
				break;
			case TRAILER:
				trailerFields.put(field, defaultValue);
				break;
			}
		}
	...
	public void setMsgFields(Map<String, String> fields) throws Exception {
		setFieldsHelper(fields, MessageFieldType.MESSAGE);
	}

	public void setHeaderFields(Map<String, String> fields) throws Exception {
		setFieldsHelper(fields, MessageFieldType.HEADER);
	}

	public void setTrailerFields(Map<String, String> fields) throws Exception {
	...
	/**
	 * The fields are of form: <fieldName>(predicate)=<fieldValue> Where
	 * fieldName is an integer number. So we parse out the field name, store it
	 * as an int, and store the value as an object. The predicate is optional
	 * 
	 * @param fields
	 *            Map of key-value pairs
	 * @param fieldType
	 *            Which particular kind of field we are modifying:
	 *            trailer/header/message
	 */
	protected void setFieldsHelper(Map<String, String> fields, DefaultMessageModifier.MessageFieldType fieldType)
		...
		for (String oneKey : keys) {
			...
			if (defaultFieldsMatcher.matches()) {
				...
				throw new Exception("Wrong order modifier format for field " + oneKey);
			}
		}
	}
}
